...
# Fonction pour gérer la redirection vers la page de gestion des menus
...
def main():
    ...
    if verify_login(username, password):
        ...
        # Section pour la liste des aliments à acheter
        ...
        # Création de colonnes pour les boutons "Gérer les menus" et "Gérer les aliments"
        col1, col2 = st.columns(2)
        
        # Bouton pour gérer les menus dans la première colonne
        with col1:
            if st.button("Gérer les menus"):
                gerer_menus()
        
        # Bouton pour gérer les aliments dans la deuxième colonne
        with col2:
...
def gerer_aliments():
    ...
    with st.form("ajout_aliment"):
        ...
        if soumettre_ajout:
            ...
            aliments_df = pd.concat([aliments_df, {'Produit': nouveau_produit, 'Quantité dispo': nouvelle_quantite, 'Unité': nouvelle_unite}], ignore_index=True)
