...
# Fonction principale pour l'affichage des pages
def main():

    if 'current_page' not in st.session_state:
        st.session_state.current_page = 'main'

    # Affichage de la page de gestion des menus
    if st.session_state.current_page == 'manage_menus':
        st.title("Gestion des Menus")
        gerer_menus()  # Ici, tu peux appeler ta fonction gerer_menus

        # Bouton de retour à la page principale
        if st.button("Retour à la page principale"):
            navigate_to('main')
    # Bouton pour aller à la page de gestion des menus
    elif st.session_state.current_page == 'main':
        # st.title("Page principale")
        if verify_login(username, password):
            if st.button("Gérer les menus"):
                navigate_to('manage_menus')
...
if verify_login(username, password):
    ...
    gest_menu = st.button("Gérer les menus")
    if gest_menu:
        gerer_menus()
