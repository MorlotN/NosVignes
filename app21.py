    ...
    with st.expander("Ajouter un nouveau menu"):
        with st.form(key='form_ajout_menu'):
            ...
            if bouton_ajouter:
                ...
                enregistrer_action(
                    type_action="Création de menu",
                    utilisateur="admin",
                    menu="Menu d'été",
                    details_action="Menu d'été ajouté avec succès"
                )
...
def calculate_required_ingredients(selected_menu, num_people, menu_df, aliments_df):
    ...
    if not menu_selected_row.empty:
        ...
        quantities_per_person = [float(q) for q in quantities_per_person_str.split(", ")]
...
def enregistrer_action(type_action, utilisateur=None, menu=None, nombre_personnes=None, produit=None, quantite=None, date_reservation=None, details_action=None):
    # Construction de l'enregistrement d'action
    action = pd.DataFrame({
        ...
        "Détails action": [details_action or "N/A"]  # Nouveau champ pour les détails supplémentaires
    })
    # Sauvegarde ou mise à jour dans le CSV
    if not os.path.isfile(chemin_historique):
